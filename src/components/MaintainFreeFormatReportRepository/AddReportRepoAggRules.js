...
import AggRepoRuleAssist from './AggRepoRuleAssist';
require('../MaintainReportRules/MaintainReportRules.css');

class AddReportRepoAggRules extends Component {
    constructor(props) {
        super(props);
        this.state = {
          display:null,
  ...
  handleSubmit(event) {
    ...
    let audit_info={
      ...
      change_reference:`Repository Aggregation Rule of : ${this.state.form.report_id}->${this.state.form.sheet_id}->${this.state.form.cell_id}`,
...
const VisibleAddReportRepoAggRules = connect(
  mapStateToProps,
  mapDispatchToProps
)(AddReportRepoAggRules);

export default VisibleAddReportRepoAggRules;
